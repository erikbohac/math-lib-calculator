...
TEST(MathLibTest, AdditionDecimal)
{
	...
	EXPECT_NEAR(add(3.42L, 1.58L), 5L, TOL_EXACT);
	EXPECT_NEAR(add(3.125'453L, -3.125'453L), 0L, TOL_EXACT);
...
TEST(MathLibTest, SubtractionDecimal)
{
	...
	EXPECT_NEAR(subtract(-53453.76L, -53452.76L), -1L, TOL_EXACT);
...
{
	EXPECT_NEAR(multiplicate(1L, 1L), 1L, TOL_EXACT);
	EXPECT_NEAR(multiplicate(10L, 0L), 0L, TOL_EXACT);
	EXPECT_NEAR(multiplicate(0L, 10L), 0L, TOL_EXACT);
	EXPECT_NEAR(multiplicate(1'000'000L, 1'000'000'000'000L), 1'000'000'000'000'000'000L, TOL_HUGE);
}

TEST(MathLibTest, MultiplicationDecimal)
{
	EXPECT_NEAR(multiplicate(2.1002L, 3.1003L), 6.511'250'06L, TOL_SMALL);
	EXPECT_NEAR(multiplicate(0L, 9.876'543'21L), 0L, TOL_EXACT);
	EXPECT_NEAR(multiplicate(123'456.789L, 987.654'321L), 121'932'631.112'635'269L, TOL_HUGE);
}
...
TEST(MathLibTest, PowerBaseDecimal)
{
	...
	EXPECT_NEAR(power(100.001L, 4L), 10004.000'600'040'001L, TOL_SMALL);
...
TEST(MathLibTest, Modulo)
{
	...
	EXPECT_NEAR(modulo(6'242'463'545L, 745'345L), 199'170L, TOL_MED);
	EXPECT_NEAR(modulo(LLONG_MAX, LLONG_MIN), -1L, TOL_EXACT);
}

TEST(MathLibTest, ModuloDecimal)
{
	EXPECT_NEAR(modulo(5.5L, 2L), 1.5L, TOL_EXACT);
	EXPECT_NEAR(modulo(-5.5L, 2L), 0.5L, TOL_EXACT);
	EXPECT_NEAR(modulo(23'423'342'345.435'465'645L, 3'433'425'345.756'465'756L), 2'822'790'270.896'671'109L, TOL_LARGE);
	EXPECT_NEAR(modulo(642'254'534.753'453'456L, -345'745.643'545L), -204'874.690'388'456L, TOL_MED);
}
